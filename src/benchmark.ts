...
import { Writer as WriterXWriter } from './index.js'
...
async function benchmark(data: string, msg: string): Promise<void> {
  ...
  const stenoxLabel = '  writerx (local)       '
  
  const fsFile = path.join(dir, 'fs.txt')
  const stenoFile = path.join(dir, 'steno.txt')
  const stenoxFile = path.join(dir, 'writerx.txt')
  
  const steno = new StenoWriter(stenoFile)
  const stenox = new WriterXWriter(stenoxFile)
  ...
  console.log('    fs = writerx               ', fsContent === stenoxContent ? '✓' : '✗')
